...
def chattering_goals(reward="kl"):
    ...
    for idx in range(N):
        ...
        # plot
        ax.clear()
        overlay_timesteps(ax, xh_traj, xr_traj, n_steps=idx, linewidth=3)
        ax.scatter(xh0[0], xh0[2], c="blue", s=150)
        ax.scatter(xr0[0], xr0[2], c="red", s=150)
    ...
    # save figure to tikz
    # tikzplotlib.save("chattering_goals.tex", strict=True)
    plt.show()

def cbp_probs(reward="kl"):
    np.random.seed(0)
    ...
    h_goal_idxs = []
    r_goal_idxs = []
    all_post_beliefs = np.zeros((goals.shape[1], goals.shape[1], 0))
    posts = np.array([np.ones(3) / 3 for _ in range(3)])
    all_post_beliefs = np.dstack((all_post_beliefs, posts))

    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(12,3))
    axes = axes.flatten()
    ax = axes[0]
    belief_ax = axes[1]
    goal_colors = ["#3A637B", "#C4A46B", "#FF5A00"] # blue=0, brown=1, orange=2
    ...
    for idx in range(N):
        ...
        for goal_idx in range(goals.shape[1]):
            goal = goals[:,[goal_idx]]
            # compute CBP belief update
            r_belief_post = r_belief.weight_by_score(r_belief_prior, goal, xh_next, beta=0.5, w2=3)
            posts.append(r_belief_post)
        all_post_beliefs = np.dstack((all_post_beliefs, np.array(posts)))
        ...
        belief_ax.clear()
        # blue=0, brown=1, orange=2
        linestyles = ["-.", "--", ":"]
        for goal_idx in range(goals.shape[1]):
            r_belief_i = all_post_beliefs[goal_idx,:,:]
            belief_ax.plot(r_belief_i[0], label="P(g0)", c=goal_colors[0], linestyle=linestyles[goal_idx])
            belief_ax.plot(r_belief_i[1], label="P(g1)", c=goal_colors[1], linestyle=linestyles[goal_idx])
            belief_ax.plot(r_belief_i[2], label="P(g2)", c=goal_colors[2], linestyle=linestyles[goal_idx])
        # plot nominal belief with different line + low alpha
        for goal_idx in range(goals.shape[1]):
            belief_ax.plot(r_beliefs_nominal[:,goal_idx], c=goal_colors[goal_idx], linestyle="-", alpha=0.5)
...
def influence_objective(mode="with_robot"):
    ...
    for idx in range(N):
        ...
        if mode == "with_robot":
            ...
            overlay_timesteps(ax, xh_traj, [], n_steps=idx, linewidth=3)
        ax.scatter(xh0[0], xh0[2], c="blue", s=150)
        ax.scatter(goals[0], goals[2], c=goal_colors, s=150)
        ax.set_xlim(-10, 10)
        # ax.set_ylim(-10, 0)

...
class CBPEstimator():
    ...
    def score_thetas(self, theta1, theta2, theta_prior, w=2):
    ...
    def weight_by_score(self, prior_belief, theta_r, beta=1):
        ...
        for i in range(self.thetas.shape[1]):
            ...
            for j in range(self.thetas.shape[1]):
                ...
                # scores = [self.score_thetas(self.thetas[:,[k]], theta_r, theta_prior) for k in range(self.thetas.shape[1])]
            ...
            belief_post[i] = np.sum(likelihood)
...
if __name__ == "__main__":
    ...
    h_belief = BayesEstimator(thetas=goals, dynamics=r_dynamics, beta=1)
    ...
    h_belief_ax = axes[1]
    r_belief_ax = axes[2]
    # TODO: plot the human's true intended goal at each timestep
    ...
    for idx in range(N):
        ...
        r_belief_post = r_belief.weight_by_score(r_belief_prior, r_goal, beta=0.1)
        # r_belief_post = r_belief.weight_by_score(r_belief_nominal.belief, r_goal, beta=2) # testing
